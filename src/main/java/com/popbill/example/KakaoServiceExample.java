...
public class KakaoServiceExample {
	...
	public String getPlusFriendMgtURL(Model m) {
		/*
		 * 카카오톡 채널을 등록하고 내역을 확인하는 카카오톡 채널 관리 페이지 팝업 URL을 반환합니다. - 반환되는 URL은 보안 정책상 30초
		 * 동안 유효하며, 시간을 초과한 후에는 해당 URL을 통한 페이지 접근이 불가합니다. -
		 * https://developers.popbill.com/reference/kakaotalk/java/api/channel#
		 * GetPlusFriendMgtURL
		 */
		try {
	...
	public String checkSenderNumber(Model m) {
		 ...
		 * https://developers.popbill.com/reference/kakaotalk/java/api/sendnum#
		 * CheckSenderNumber
		 */
		try {
	...
	public String getSenderNumberMgtURL(Model m) {
		/*
		 * 발신번호를 등록하고 내역을 확인하는 카카오톡 발신번호 관리 페이지 팝업 URL을 반환합니다. - 반환되는 URL은 보안 정책상 30초 동안
	...
	public String getATSTemplateMgtURL(Model m) {
		/*
		 * 알림톡 템플릿을 신청하고 승인심사 결과를 확인하며 등록 내역을 확인하는 알림톡 템플릿 관리 페이지 팝업 URL을 반환합니다. - 반환되는
		 * URL은 보안 정책상 30초 동안 유효하며, 시간을 초과한 후에는 해당 URL을 통한 페이지 접근이 불가합니다. -
		 * https://developers.popbill.com/reference/kakaotalk/java/api/template#
	...
	public String listATSTemplate(Model m) {
		/*
		 * 승인된 알림톡 템플릿 목록을 확인합니다. - 반환항목중 템플릿코드(templateCode)는 알림톡 전송시 사용됩니다. -
		 * https://developers.popbill.com/reference/kakaotalk/java/api/template#
		 * ListATSTemplate
		 */
	...
	public String sendATS_one(Model m) {
		/*
		 * 승인된 템플릿의 내용을 작성하여 1건의 알림톡 전송을 팝빌에 접수합니다. - 사전에 승인된 템플릿의 내용과 알림톡
		 * 전송내용(content)이 다를 경우 전송실패 처리됩니다. - 전송실패 시 사전에 지정한 변수 'altSendType' 값으로 대체문자를
		 * 전송할 수 있고 이 경우 문자(SMS/LMS) 요금이 과금됩니다. -
...
//        btns[0] = button;
		...
		try {
			...
			String receiptNum = kakaoService.sendATS(testCorpNum, templateCode, senderNum, content, altSubject,
					altContent, altSendType, receiverNum, receiverName, sndDT, testUserID, requestNum, btns);
	...
	public String sendATS_multi(Model m) {
		 ...
		 * 사전에 승인된 템플릿의 내용과 알림톡 전송내용(content)이 다를 경우 전송실패 처리됩니다. - 전송실패 시 사전에 지정한 변수
		 * 'altSendType' 값으로 대체문자를 전송할 수 있고, 이 경우 문자(SMS/LMS) 요금이 과금됩니다. -
...
//        btns[0] = button;
	...
	public String sendATS_same(Model m) {
		 ...
		 * 사전에 승인된 템플릿의 내용과 알림톡 전송내용(content)이 다를 경우 전송실패 처리됩니다. - 전송실패시 사전에 지정한 변수
		 * 'altSendType' 값으로 대체문자를 전송할 수 있고, 이 경우 문자(SMS/LMS) 요금이 과금됩니다. -
...
//        btns[0] = button;
	...
	public String sendFTS_one(Model m) {
		/*
		 * 텍스트로 구성된 1건의 친구톡 전송을 팝빌에 접수합니다. - 친구톡의 경우 야간 전송은 제한됩니다. (20:00 ~ 익일 08:00) -
		 * 전송실패시 사전에 지정한 변수 'altSendType' 값으로 대체문자를 전송할 수 있고, 이 경우 문자(SMS/LMS) 요금이
		 * 과금됩니다. -
		...
		try {
			...
			String receiptNum = kakaoService.sendFTS(testCorpNum, plusFriendID, senderNum, content, altSubject,
					altContent, altSendType, btns, receiverNum, receiverName, sndDT, adsYN, testUserID, requestNum);
	...
	public String sendFTS_multi(Model m) {
		/*
		 * 텍스트로 구성된 다수건의 친구톡 전송을 팝빌에 접수하며, 수신자 별로 개별 내용을 전송합니다. (최대 1,000건) - 친구톡의 경우 야간
		 * 전송은 제한됩니다. (20:00 ~ 익일 08:00) - 전송실패시 사전에 지정한 변수 'altSendType' 값으로 대체문자를 전송할
		 * 수 있고, 이 경우 문자(SMS/LMS) 요금이 과금됩니다. -
	...
	public String sendFTS_same(Model m) {
		/*
		 * 텍스트로 구성된 다수건의 친구톡 전송을 팝빌에 접수하며, 모든 수신자에게 동일 내용을 전송합니다. (최대 1,000건) - 친구톡의 경우
		 * 야간 전송은 제한됩니다. (20:00 ~ 익일 08:00) - 전송실패시 사전에 지정한 변수 'altSendType' 값으로 대체문자를
		 * 전송할 수 있고, 이 경우 문자(SMS/LMS) 요금이 과금됩니다. -
	...
	public String sendFMS_one(Model m) {
		/*
		 * 이미지가 첨부된 1건의 친구톡 전송을 팝빌에 접수합니다. - 친구톡의 경우 야간 전송은 제한됩니다. (20:00 ~ 익일 08:00) -
		 * 전송실패시 사전에 지정한 변수 'altSendType' 값으로 대체문자를 전송할 수 있고, 이 경우 문자(SMS/LMS) 요금이
		 * 과금됩니다. - 대체문자의 경우, 포토문자(MMS) 형식은 지원하고 있지 않습니다. -
		...
		try {
			...
			String receiptNum = kakaoService.sendFMS(testCorpNum, plusFriendID, senderNum, content, altSubject,
					altContent, altSendType, btns, receiverNum, receiverName, sndDT, adsYN, file, imageURL, testUserID,
					requestNum);
	...
	public String sendFMS_multi(Model m) {
		/*
		 * 이미지가 첨부된 다수건의 친구톡 전송을 팝빌에 접수하며, 수신자 별로 개별 내용을 전송합니다. (최대 1,000건) - 친구톡의 경우 야간
		 * 전송은 제한됩니다. (20:00 ~ 익일 08:00) - 전송실패시 사전에 지정한 변수 'altSendType' 값으로 대체문자를 전송할
		 * 수 있고, 이 경우 문자(SMS/LMS) 요금이 과금됩니다. - 대체문자의 경우, 포토문자(MMS) 형식은 지원하고 있지 않습니다. -
...
//            message.getBtns().add(button02);
	...
	public String sendFMS_same(Model m) {
		/*
		 * 이미지가 첨부된 다수건의 친구톡 전송을 팝빌에 접수하며, 모든 수신자에게 동일 내용을 전송합니다. (최대 1,000건) - 친구톡의 경우
		 * 야간 전송은 제한됩니다. (20:00 ~ 익일 08:00) - 전송실패시 사전에 지정한 변수 'altSendType' 값으로 대체문자를
		 * 전송할 수 있고, 이 경우 문자(SMS/LMS) 요금이 과금됩니다. - 대체문자의 경우, 포토문자(MMS) 형식은 지원하고 있지 않습니다.
		 * -
		 * https://developers.popbill.com/reference/kakaotalk/java/api/send#SendFMSSame
		 */
		...
		try {
			...
			String receiptNum = kakaoService.sendFMS(testCorpNum, plusFriendID, senderNum, content, altSubject,
					altContent, altSendType, receivers, btns, sndDT, adsYN, file, imageURL, testUserID, requestNum);
	...
	public String cancelReserve(Model m) {
		 ...
		 * https://developers.popbill.com/reference/kakaotalk/java/api/send#
		 * CancelReserve
		 */
	...
	public String getMessagesRN(Model m) {
		 ...
		 * https://developers.popbill.com/reference/kakaotalk/java/api/info#
		 * GetMessagesRN
		 */
	...
	public String search(Model m) {
		/*
		 * 검색조건에 해당하는 카카오톡 전송내역을 조회합니다. (조회기간 단위 : 최대 2개월) - 카카오톡 접수일시로부터 6개월 이내 접수건만
		 * 조회할 수 있습니다. -
	...
	public String getSentListURL(Model m) {
		/*
		 * 카카오톡 전송내역을 확인하는 페이지의 팝업 URL을 반환합니다. - 반환되는 URL은 보안 정책상 30초 동안 유효하며, 시간을 초과한
	...
	public String chargeInfo(Model m) {
		 ...
		 * https://developers.popbill.com/reference/kakaotalk/java/api/point#
		 * GetChargeInfo
		 */
